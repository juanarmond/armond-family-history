...
- Audited the whole tree behind it: the 41 people without a parentage family (four parallel agents) yielded **zero un-modelled gaps** — every subject whose held record names parents already has them noded, and the rest are genuinely top-of-line (named only in a descendant's record, or their parents exist solely as un-promoted published-genealogy / collaborative-tree leads that must not be minted into nodes). The José do Rego Brandão line specifically is complete: the 1912 death PAR-0008 is the *son's* (P-0040) record and already yields the elder José (P-0043) × Maria de Mendonça (P-0044) as family F-0021.
...
- **Scales to deep ancestry** without a full pedigree's exponential blow-up: columns are packed to the ancestry's real width (a sparse deep line becomes a narrow chain rather than a scatter across empty 2^gen columns), and the all-branches view is capped at 6 generations. **Every card with known ancestry carries a ＋ / − toggle** — collapse any branch to focus the view, or open a single deep line past the base limit — and each branch opens or closes independently without disturbing the rest, so you shape the tree to what you want to see. Zoom is clamped to 25–100% and cannot drop below the fit-to-width level (zooming out past the point where the tree already fits would only add empty margin). Touched `family-tree-viewer/{app,styles,i18n,index.html}`.
